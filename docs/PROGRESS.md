...
- `docs/ROADMAP.md`, `docs/PROGRESS.md`, `CONTRIBUTING.md`, `.gitignore`
...
|---|---|---|---|
| 2026-09-03 | **`analyzeContour` upload field renamed `file` → `contour_map`** (route param, frontend `FormData`, tests, cookbook, report curl examples, `docs/api/openapi.json`) | Root cause of a 5-mark grading deduction: the class TA's posted spec requires the multipart field be named `contour_map`; the automated grader's request lacked our `file` field entirely, and FastAPI correctly 422'd with `loc: ["body","file"], msg: "Field required"`. Reproduced the exact 422 locally with an empty/JSON POST; a real multipart upload under the old name worked end-to-end (202 → succeeded), so the endpoint itself was never broken — only the wire field name didn't match the grader's contract | Leaving the field as `file` and disputing the deduction (the TA's instruction is explicit and the fix is free); accepting both names via an alias (unnecessary surface once the spec is known) |
| 2026-09-01 | **Flood-belt buffer**: no candidate within 200 m (configurable) of a channel beyond the ideal band — a second hard tier on top of the channel exclusion | Candidates 30–67 m from the sample's 416 ha channel were legal but sat in its flood belt; a spate overtops a bund there. Found by the user reading the map, not by a test — the belt makes the visual expectation ("not on the river") an enforced rule | Trusting NMS + the plateau to keep sites off the banks (they don't when tributary mouths score well) |
