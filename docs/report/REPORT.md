...
|---|---|---|
| **A river already crosses the area** | Two hard exclusions, not a low score. (1) Channel cells at or beyond the plateau's upper bound (1 000 ha default) are excluded outright — with no better cell available a soft score alone would still put the pond on the river. (2) A **flood-belt buffer**: nothing within 200 m (configurable) of any channel beyond the 10–150 ha ideal band is a candidate either — a bund 30 m from a 400 ha channel would be overtopped by its spates. Any such channel triggers the `existing_watercourse` warning naming its size and the setback. On the sample, 157 cells on or near the ~416 ha watercourse are excluded and every candidate keeps 469–1 466 m clear of it. | golden tests `…river_is_excluded…` and `…flood_belt…`; figure below |
| Flat / featureless / tiny terrain | No drainage network forms → siting returns an empty candidate list and the route still **succeeds**: terrain products are returned with the `no_site_found` warning, the catchment search widens once to the whole map (`catchment_from_map_outlet`), and if no cell drains the 2 ha minimum the result carries `catchment: null` + `catchment_unavailable` — never a failed job | golden test `test_flat_terrain_yields_no_candidates…`; API test `test_a_map_with_no_drainage_still_returns_terrain_not_a_failure` |
